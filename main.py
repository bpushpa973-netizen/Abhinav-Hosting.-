import os, zipfile, subprocess, signal, shutil, psutil, time, datetime
...
# Initialize SocketIO with threading mode (compatible with gunicorn default worker)
socketio = SocketIO(async_mode='threading')

class _DB:
    """Thin wrapper so existing code (db.execute(...).fetchone()) works with psycopg2."""
    ...
    def execute(self, query, params=None):
        pg = query.replace('?', '%s')
        self._cur.execute(pg, params or ())
        return self._cur
...
def init_db():
    ...
    db.execute('''INSERT INTO admin_settings (id, username, password)
        VALUES (1, 'abhinav', '1100') ON CONFLICT (id) DO NOTHING''')
    db.commit()
    db.close()
...
def create_app():
    ...
    def upload_file(folder):
        ...
        # Verify folder belongs to the logged-in user
        db = get_db()
        srv = db.execute('SELECT * FROM servers WHERE folder=? AND user_id=?', (folder, session['user_id'])).fetchone()
        db.close()
        if not srv:
        ...
        base = app.config['BASE_STORAGE']
        dest = os.path.normpath(os.path.join(base, folder, sub_path))
        # Prevent path traversal — dest must stay inside BASE_STORAGE
    ...
    def server_action(folder, act):
        ...
        def ensure_venv(log_handle):
            if not os.path.exists(venv_python):
                log_handle.write(f"[{now}] 🔧 Creating virtual environment...\n")
                log_handle.flush()
                subprocess.run(['python3', '-m', 'venv', venv_path], cwd=path)
                log_handle.write(f"[{now}] ✅ Virtual environment ready.\n")
                log_handle.flush()
        ...
        if act == 'install':
            ...
            if os.path.exists(req_path):
                ...
                f_log.flush()
                ensure_venv(f_log)
                subprocess.Popen([venv_pip, 'install', '-r', 'requirements.txt'], cwd=path, stdout=f_log, stderr=f_log)
                db.close()
        ...
        if act in ['start', 'restart']:
            ...
            if os.path.exists(req_path):
                ensure_venv(f_log)
                f_log.write(f"[{now}] 📦 Auto-installing requirements.txt...\n")
                f_log.flush()
                subprocess.run([venv_pip, 'install', '-r', 'requirements.txt'], cwd=path, stdout=f_log, stderr=f_log)
                f_log.write(f"[{now}] ✅ Packages ready.\n")
                f_log.flush()
            f_log.write(f"\n[{now}] 🚀 Instance {act.upper()}ED Successfully\n")
            f_log.flush()
            python_bin = venv_python if os.path.exists(venv_python) else 'python3'
            proc = subprocess.Popen([python_bin, startup_file], cwd=path, stdout=f_log, stderr=f_log, preexec_fn=os.setsid)
    ...
    def server_command(folder):
        ...
        if not cmd:
            return jsonify({'status': 'error', 'msg': 'No command'})
        path = os.path.join(app.config['BASE_STORAGE'], folder)
        log_file_path = os.path.join(path, 'console.log')
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        try:
